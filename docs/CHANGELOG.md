...
- Fixed a crash on every startup affecting MSVC-built executables (the MinGW build was unaffected): the startup call chain nests many functions that each hold one or more ~64 KB path buffers on the stack, together exceeding the 1 MB stack that MSVC's linker reserves by default — MinGW's linker reserves 2 MB, which is the only reason its build survived. The thread stack overflowed before the main window finished being created, which Windows Error Reporting records as an opaque `0xc000041d` crash with no faulting detail. Both build systems now explicitly reserve an 8 MB stack (`/STACK` for MSVC, `--stack` for MinGW), and four of the largest buffers were moved out of the window procedure's shared stack frame into helper functions. (`c0a2306`)
